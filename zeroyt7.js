...
owner = '6282273112644' //Ganti Nomor Luh
botname = 'SADBOTZ' //Ganti Nama Bot Luh
zerokey = '568e544acee6d266eded7bc7' //Gak Usah Di Ganti
ownername = 'XRX' //Ganti Nama Owner
zerkey =  "ZeroYT7"
...
module.exports = zeroyt7 = async (zeroyt7, mek, _welkom) => {
	try {
		...
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6282273112644@s.whatsapp.net`]
...
"body": `hy`, 
"mediaType": "2", 
"mediaUrl": "https://youtu.be/SC1JsThUrRw", 
	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `𝘊𝘳𝘦𝘢𝘵𝘰𝘳 : XRX 么`,  //Ganti Aja Nama Luh
                            orderTitle: `𝘊𝘳𝘦𝘢𝘵𝘰𝘳 : XRX 么`,
        ...
        const fakeitem = (teks) => {
           zeroyt7.sendMessage(from, teks, text, {
                   quoted: {
                           ...
                           },message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync("./media/Menu.jpg"),"itemCount":123,"status":"INQUIRY","surface":"CATALOG","message": `𝘊𝘳𝘦𝘢𝘵𝘰𝘳 : XRX 么`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}},
...
case 'command':
               list = []
               listmenu = [`order`,`sewabot`,`grupmenu`,`stickermenu`,`downloadmenu`,`islammenu`,`animemenu`,`informationmenu`,`randomtextmenu`,`searchmenu`,`primbonmenu`,`stalkmenu`,`randomimagemenu`,`ownermenu`,`funmenu`,]
               listmenuu = [`Order`,`Sewa Bot`,`Grup Menu`,`Sticker Menu`,`Download Menu`,`Islam Menu`,`Anime Menu`,`Information Menu`,`Random Text Menu`,`Search Menu`,`Primbon Menu`,`Stalk Menu`,`Random Image Menu`,`Owner Menu`,`funmenu`,]
               nombor = 1
               startnum = 0
...
✎ ${prefix}infobot
✎ ${prefix}sc
✎ ${prefix}mode [self/public]button
✎ ${prefix}runtime>
✎ ${prefix}report`
fakeitem(menu)
break
...
costum(buffer, sticker, Verived, `Powered by SadBotZ`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `SADBOTZ WhatsApp Bot`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
zeroyt7.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Powered by SadBotZ' })
fs.unlinkSync(ran)
})
break
...
break
case 'ytmp3':
if (args.length == 0) return reply(`Link Nya Mana Tod\nContoh: ${prefix + command} https://www.youtube.com/watch?v=qHIQAk-BCyEc`)
ini_link = args[0]
...
reply(mess.wait)
zeroyt7.sendMessage(from, gambar, image, { quoted: ftrol, caption: 'Nih Powered by SadBotZ'})
})
break
case 'chiisaihentai':
case 'trap':
...
for (let _ of anu) {
sendMess(_.jid, `[ *BOT XRX BROADCAST* ]\n\n${body.slice(4)}`)
}
reply('Suksess broadcast')
}
break
...
zeroyt7.sendMessage(`6282273112644@s.whatsapp.net`, options, text, { quoted: ftrol })
...
case 'sc':
hayo = `
Bot Ini Menggunakan Sc : bit.ly/scxrx
Moga work
...
sendButton(from, "SEWA PERMANEN 20K", faketeks, but, mek)
break
case 'gopay':
but = [
...
sendButton(from, "DANA : TF KE QRIS(MINTA KE OWNER)", faketeks, but, mek)
break
case 'ovo':
but = [
{ buttonId: `${prefix}done`, buttonText: { displayText: 'OWNER 🔥' }, type: 1 }
]
sendButton(from, "OVO : TF KE QRIS(MINTA KE OWNER)", faketeks, but, mek)
break
